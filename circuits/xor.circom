...
template Xor () {
  ...
  for (var i = 0; i < NUM_BITS; i++) {
    // lower
    lower_bits_to_num.in[i] <== XOR()(lower_num_to_bits[0].out[i], lower_num_to_bits[1].out[i]); // XOR gate

    // upper
    upper_bits_to_num.in[i] <== XOR()(upper_num_to_bits[0].out[i], upper_num_to_bits[1].out[i]); // XOR gate
  }
